  ...
  return (
    <>
      ...
      <section className="mb-40">
        <SectionHeading title="أحدث الإضافات" viewAllTo="/" viewAllLabel="عرض كافة المجموعات" className="mb-12" />
        
        {loading && (
          <p className="font-sans text-text-muted text-center py-12">جاري تحميل الأعمال...</p>
        )}
        {error && !loading && (
          <p className="font-sans text-red-500 text-center py-12">تعذر تحميل الأعمال: {error}</p>
        )}
        {!loading && !error && works.length === 0 && (
          <p className="font-sans text-text-muted text-center py-12">لا توجد أعمال منشورة بعد.</p>
        )}
        {!loading && !error && works.length > 0 && (
